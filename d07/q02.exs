...
needed_update_space = 30_000_000
free_space = total_space - ContentSize.total(directory_tree["/"], directory_tree, 0)
space_total_that_needs_to_be_freed = needed_update_space - free_space

# Traverse the tree fetching directory size
# filter for sizes >= space_total_that_needs_to_be_freed
# and return the smallest candidate
directory_tree
|> Enum.map(fn {dirname, node} ->
  size = ContentSize.total(node, directory_tree, 0)
  {dirname, size}
end)
|> Enum.filter(fn {_, size} -> size >= space_total_that_needs_to_be_freed end)
